(ns agri-technician.actor
  "TechnicianActor — the ISCO-08 3142 agricultural technician actor as a
  `langgraph.graph/state-graph` (per ADR-2607011000 / CLAUDE.md Actors section).
  One graph run = one technical request (intake → advise → govern → decide →
  ...
  The unconditional invariant: the TechnicianAdvisor can never
  directly commit a record or dispatch an action the TechnicianGovernor
  refuses — every commit-record! call is gated behind `:decide`."
  (:require [langgraph.graph :as g]
            [langgraph.checkpoint :as cp]
            [agri-technician.advisor :as advisor]
...
(defn build-graph
  ...
  `agri-technician.store/Store`. `advisor` implements
  `agri-technician.advisor/Advisor` (defaults to `mock-advisor`).
  ...
  (-> (g/state-graph
      ...
      (g/add-node :commit
                   (fn [{:keys [request proposal]}]
                     (let [record {:farm-id (:farm-id request)
                                    :op (:op proposal)
                                    :payload proposal}]
